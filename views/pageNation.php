<?php if($page_nation['current_page'] > 1): ?>
  <a href="<?= $base_url; ?>/<?= $path; ?>?page=<?= $this->escape($page_nation['current_page'] - 1); ?>
  <?php if($path === '' || $path === 'recipe'): ?>
  &freeword=<?= $this->escape($_GET['freeword']); ?>&category=<?= $this->escape($_GET['category']); ?>&sort=<?= $this->escape($_GET['sort']); ?>
  <?php endif; ?>
  ">前へ</a>
<?php else: ?>
  <span>前へ</span>
...
<?php for($i = $page_nation['range'] + $page_nation['next_diff']; $i > 0; $i--): ?>
  ...
  <a href="<?= $base_url; ?>/<?= $path; ?>?page=<?= $this->escape($page_nation['current_page'] - $i); ?>
  <?php if($path === '' || $path === 'recipe'): ?>
  &freeword=<?= $this->escape($_GET['freeword']); ?>&category=<?= $this->escape($_GET['category']); ?>&sort=<?= $this->escape($_GET['sort']); ?>
  <?php endif; ?>
  ">
